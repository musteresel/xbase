...
xbShort xbExpn::ProcessExpression(xbExpNode * Wtree, xbShort RecBufSw) {
	...
	while (WorkNode) {
		...
		if (WorkNode->Type == 'D' && WorkNode->dbf) {
			WorkNode->dbf->GetField(
			    WorkNode->FieldNo, WorkNode->StringResult, RecBufSw);
			if (WorkNode->dbf->GetFieldType(WorkNode->FieldNo) == 'N' ||
			    WorkNode->dbf->GetFieldType(WorkNode->FieldNo) == 'F')
				WorkNode->DoubResult =
				    WorkNode->dbf->GetDoubleField(WorkNode->FieldNo, RecBufSw);
	...
	*/
	...
	if (e->Type == 'C') {
		if (e->NodeText[0] == '-' || e->NodeText[0] == '+' ||
		    (isdigit(e->NodeText[0]) && !(e->NodeText[e->DataLen] == '\'' ||
		                                  e->NodeText[e->DataLen] == '"')))
			return 'N';
		else
			return 'C';
...
xbShort xbExpn::ProcessOperator(xbShort RecBufSw) {
	...
	if (OpType1 == 'N' || OpType1 == 'L' ||
	    OpType1 == 'D') /* numeric procesing */
...
xbShort xbExpn::AlphaOperation(char * Operator) {
	...
	/* not = */
	else if ((strncmp(Operator, "<>", 2) == 0) || Operator[0] == '#' ||
	         strncmp(Operator, "!=", 2) == 0)
