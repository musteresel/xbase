...
xbShort xbDbf::ReadMemoBlock(xbLong BlockNo, xbShort Option) {
	...
	if (Option == 0 ||
	    Option == 4) {  // 1st block of a set of valid data blocks
...
//  try {
#endif
	if (_fseek(mfp, ((xbOffT)BlockNo * MemoHeader.BlockSize + 4), SEEK_SET) !=
	    0)
		return XB_SEEK_ERROR;
	char h[4];
...
/*!
  ...
  \param LockOpt
*/
xbShort xbDbf::GetMemoField(xbShort FieldNo,
                            xbLong len,
                            char * Buf,
                            xbShort LockOpt) {
	if (Version == (char)0xf5 || Version == (char)0x30)
...
xbShort xbDbf::DeleteMemoField(xbShort FieldNo) {
	...
	if ((SBlockNo + SNoOfBlocks) == NextFreeBlock &&
	    NextFreeBlock < LastDataBlock) {
		if ((rc = ReadMemoBlock(NextFreeBlock, 2)) != XB_NO_ERROR)
			return XB_NO_ERROR;
...
xbShort xbDbf::AddMemoData(xbShort FieldNo, xbLong DataLen, const char * Buf) {
	xbShort rc;
	...
	if (IsType3Dbt() ||                            /* always append to end */
		...
		if ((rc = FindBlockSetInChain(
		         BlocksNeeded, LastDataBlock, HeadBlock, PrevNode)) == 1) {
			if ((rc = GetBlockSetFromChain(
			         BlocksNeeded, HeadBlock, PrevNode)) != XB_NO_ERROR)
				return rc;
...
*/
xbShort xbDbf::PutMemoData(xbLong StartBlock,
                           xbLong BlocksNeeded,
                           xbLong DataLen,
                           const char * Buf) {
	xbShort i, rc, Tctr;
...
xbShort xbDbf::UpdateMemoData(xbShort FieldNo,
	...
	if (DataLen) {
		...
		if (IsType4Dbt())
			TotalLen += 8;  //  leading fields for dbase iv
	} else
		TotalLen = 0;
	...
	if (DataLen == 0L) { /* handle delete */
		...
		if (BlocksNeeded == BlocksAvailable) {
			if ((rc = PutMemoData(
			         GetLongField(FieldNo), BlocksNeeded, DataLen, Buf)) !=
			    XB_NO_ERROR) {
#ifdef XB_LOCKING_ON
			//        LockMemoFile( XB_UNLOCK );
#endif
