...
int xbDate::CalcRollingCenturyForYear(int CalcYear) const {
	...
	else if (ThisYear >= 80 && CalcYear < ThisYear &&
...
int xbDate::DayOf(int Format, const char * Date8) const {
	...
	if (Format != XB_FMT_WEEK && Format != XB_FMT_MONTH &&
	    Format != XB_FMT_YEAR)
	...
	if (Format == XB_FMT_WEEK) {
		...
		iday2 = ((13 * imonth - 1) / 5) + iday + (iyear % 100) +
		        ((iyear % 100) / 4) + ((iyear / 100) / 4) - 2 * (iyear / 100) +
		        77;
		...
		return (AggregatedDaysInMonths[IsLeapYear(Date8)][MonthOf(Date8) - 1] +
...
xbString & xbDate::Sysdate() {
	...
	tblock->tm_mon++;
	sprintf(
	    dt, "%4d%02d%02d", tblock->tm_year, tblock->tm_mon, tblock->tm_mday);
	dt[8] = 0x00;
	cDate8 = dt;
	return cDate8;
}
...
xbString & xbDate::FormatCTODdate(const char * indate) {
	...
	sprintf(cbuf, "%02d", d.CalcRollingCenturyForYear(atoi(indate + 6)));
	odate[0] = cbuf[0];
	odate[1] = cbuf[1];
	odate[2] = indate[6];
	odate[3] = indate[7];
