...
/* datafile methods */
	...
	xbShort RebuildAllIndices(void (*statusFunc)(xbLong itemNum,
	                                             xbLong numItems) = 0);
	...
	/* field methods */
	const char * GetField(
	    xbShort FieldNo) const;  // Using internal static buffer
...
//  xbShort   GetMemoLockCnt() { return MemoLockCnt; }
		...
		/*
		  ...
		  ); xbShort   LockXbaseRecord( xbShort LockType, xbULong RecNo, xbULong
		  RecCnt ); xbShort   LockClipperRecord( xbShort LockType, xbULong
		  RecNo, xbULong RecCnt ); xbShort   LockFoxproRecord( xbShort LockType,
		  xbULong RecNo, xbULong RecCnt ); xbShort   LockDbaseRecord( xbShort
		  LockType, xbULong RecNo, xbULong RecCnt );
...
#ifndef HAVE_FCNTL
	xbShort UnixToDosLockCommand(xbShort WaitOption, xbShort LockType) const;
#endif

#else
	xbShort LockDatabase(xbShort, xbShort, xbLong) { return XB_NO_ERROR; }
...
#ifdef XB_MEMO_FIELDS
	xbShort GetMemoField(xbShort FieldNo,
	                     xbLong len,
	                     char * Buf,
	                     xbShort LockOption);
	xbLong GetMemoFieldLen(xbShort FieldNo);
	xbShort GetFPTField(xbShort FieldNo,
	                    xbLong len,
	                    char * Buf,
	                    xbShort LockOption);
	xbLong GetFPTFieldLen(xbShort FieldNo);
	xbShort UpdateMemoData(xbShort FieldNo,
	                       xbLong len,
	                       const char * Buf,
	                       xbShort LockOption);
	...
	xbShort FindBlockSetInChain(xbLong BlocksNeeded,
	                            xbLong LastDataBlock,
	                            xbLong & Location,
	                            xbLong & PreviousNode);
	xbShort GetBlockSetFromChain(xbLong BlocksNeeded,
	                             xbLong Location,
	                             xbLong PreviousNode);
...
private:
	...
	void InitVars();
	xbShort PackDatafiles(void (*statusFunc)(xbLong itemNum,
	                                         xbLong numItems) = 0);
	...
	xbShort OpenFPTFile();
	xbShort PutMemoData(xbLong StartBlock,
	                    xbLong BlocksNeeded,
	                    xbLong Len,
	                    const char * Buf);
	...
	xbShort CurRecLockType;   /* current type of rec lock held (F_RDLOCK or
	                             F_WRLCK) */
