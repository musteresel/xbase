...
xbNodeLink * xbNtx::GetNodeMemory(void) {
	...
	if (FreeNodeChain) {
		...
		for (int i = 0; i < HeadNode.KeysPerNode + 1; i++) {
			temp->offsets[i] =
			    2 + ((HeadNode.KeysPerNode + 1) * 2) + (HeadNode.KeySize * i);
		}
	} else {
		...
		temp->offsets =
		    (xbUShort *)malloc((HeadNode.KeysPerNode + 1) * sizeof(xbUShort));
...
void xbNtx::DumpHdrNode(xbShort Option) {
	if (Option == 0) {
		...
		std::cout << "HalfKeysPerPage = " << HeadNode.HalfKeysPerNode
		          << std::endl;
		std::cout << "KeyExpression   = " << HeadNode.KeyExpression
		          << std::endl;
...
xbShort xbNtx::GetHeadNode(void) {
	...
	p = Node;
	HeadNode.Signature = dbf->xbase->GetShort(p);
	p += sizeof(xbUShort);
	HeadNode.Version = dbf->xbase->GetShort(p);
	p += sizeof(xbUShort);
	HeadNode.StartNode = dbf->xbase->GetULong(p);
	p += sizeof(xbULong);
	HeadNode.UnusedOffset = dbf->xbase->GetULong(p);
	p += sizeof(xbULong);
	HeadNode.KeySize = dbf->xbase->GetShort(p);
	p += sizeof(xbUShort);
	HeadNode.KeyLen = dbf->xbase->GetShort(p);
	p += sizeof(xbUShort);
	HeadNode.DecimalCount = dbf->xbase->GetShort(p);
	p += sizeof(xbUShort);
	HeadNode.KeysPerNode = dbf->xbase->GetShort(p);
	p += sizeof(xbUShort);
	HeadNode.HalfKeysPerNode = dbf->xbase->GetShort(p);
	p += sizeof(xbUShort);
	strncpy(HeadNode.KeyExpression, p, 256);
	p += 256;
...
xbLong xbNtx::GetDbfNo(xbShort RecNo, xbNodeLink * n) {
	...
	temp = &n->Leaf;
	p = temp->KeyRecs;
	if (RecNo < 0 || RecNo > (temp->NoOfKeysThisNode))
		return 0L;
...
/*!
  ...
  \param
*/
xbUShort xbNtx::GetItemOffset(xbShort RecNo, xbNodeLink * n, xbShort) {
...
/*!
  ...
  \param n
*/
xbUShort xbNtx::InsertKeyOffset(xbShort pos, xbNodeLink * n) {
	xbUShort temp;
...
/*!
  ...
  \param n
*/
xbUShort xbNtx::DeleteKeyOffset(xbShort pos, xbNodeLink * n) {
	xbUShort temp;
	xbShort i;
	// save the old offset
	temp = n->offsets[pos];
...
xbShort xbNtx::CompareKey(const char * Key1, const char * Key2, xbShort Klen) {
	...
	if (Klen > HeadNode.KeyLen)
		Klen = HeadNode.KeyLen;
	k1 = Key1;
	k2 = Key2;
	for (i = 0; i < Klen; i++) {
		if (*k1 > *k2)
...
//            LockIndex( XB_UNLOCK );
	...
	while (rc == 0 || rc == XB_FOUND) {
		if (strncmp(Tkey,
		            GetKeyData(CurNode->CurKeyNo, CurNode),
		            HeadNode.KeyLen) == 0) {
...
//         LockIndex( XB_UNLOCK );
	...
	HeadNode.KeysPerNode =
	    (xbUShort)((XB_NTX_NODE_SIZE - (2 * sizeof(xbUShort))) /
	               (HeadNode.KeyLen + 10)) -
	    1;
	...
	//   while(( HeadNode.KeySize % 4 ) != 0 ) HeadNode.KeySize++;  /* multiple
	//   of 4*/
...
//       LockIndex( XB_UNLOCK );
	...
	for (i = 0; i < HeadNode.KeysPerNode + 1; i++)
		CurNode->offsets[i] =
		    (i * HeadNode.KeySize) + 2 + (2 * (HeadNode.KeysPerNode + 1));
...
xbShort xbNtx::TouchIndex(void) {
	xbShort rc;
	if ((rc = GetHeadNode()) != XB_NO_ERROR)
		return rc;
	HeadNode.Version++;
	if ((rc = PutHeadNode(&HeadNode, indexfp, 1)) != XB_NO_ERROR)
		return rc;
	return XB_NO_ERROR;
}
/************************************************************************/
...
xbShort xbNtx::SplitLeafNode(xbNodeLink * n1,
	...
	for (j = 0, i = HeadNode.HalfKeysPerNode; i < HeadNode.KeysPerNode;
	     i++, j++) {
		temp = n2->offsets[j];
...
xbShort xbNtx::CreateKey(xbShort RecBufSw, xbShort KeyBufSw) {
	...
	if (KeyBufSw) {
		...
		memcpy(KeyBuf2,
		       TempNode->StringResult,
		       XB_MIN(HeadNode.KeyLen + 1, TempNode->DataLen));
	} else {
...
xbShort xbNtx::UpdateParentKey(xbNodeLink * n) {
	...
	while (TempNode) {
		if (TempNode->CurKeyNo < TempNode->Leaf.NoOfKeysThisNode) {
			memcpy(KeyBuf,
			       GetKeyData(n->Leaf.NoOfKeysThisNode - 1, n),
			       HeadNode.KeyLen);
...
/*!
  ...
  \param n
*/
xbShort xbNtx::DeleteKeyFromNode(xbShort pos, xbNodeLink * n) {
	xbNodeLink * TempNode;
	xbShort rc;
	...
	if (GetLeftNodeNo(0, n) != 0) {
		...
		while ((rc = GetLeftNodeNo(0, CurNode)) != 0)
			GetLeafNode(GetLeftNodeNo(CurNode->Leaf.NoOfKeysThisNode, CurNode),
			            1);
		...
		PutDbfNo(pos,
		         TempNode,
		         GetDbfNo(CurNode->Leaf.NoOfKeysThisNode - 1, CurNode));
		...
		if ((rc = PutLeafNode(n->NodeNo, n)) != 0)
			return rc;
...
xbShort xbNtx::RemoveKeyFromNode(xbShort pos, xbNodeLink * n) {
	...
	if (!(n->NodeNo == HeadNode.StartNode) &&
		...
		if (parent->CurKeyNo == parent->Leaf.NoOfKeysThisNode) {
			...
			if ((rc = PutLeafNode(n->NodeNo, n)) != 0)
				return rc;
			if ((rc = PutLeafNode(sibling->NodeNo, sibling)) != 0)
				return rc;
			if ((rc = PutLeafNode(parent->NodeNo, parent)) != 0)
				return rc;
			...
			if (harvest) {
				...
				PutLeftNodeNo(parent->CurKeyNo + 1,
				              parent,
...
xbShort xbNtx::JoinSiblings(xbNodeLink * parent,
	...
	if ((n1->Leaf.NoOfKeysThisNode + n2->Leaf.NoOfKeysThisNode + 1) <=
	    HeadNode.KeysPerNode) {
		...
		for (i = n1->Leaf.NoOfKeysThisNode, j = 0;
		     j < n2->Leaf.NoOfKeysThisNode;
		     i++, j++) {
		...
		if (n1->Leaf.NoOfKeysThisNode > HeadNode.HalfKeysPerNode) {
			...
			// Copy up the last key from n1, that will become the new parent
			// key.
			strcpy(KeyBuf, GetKeyData(n1->Leaf.NoOfKeysThisNode - 1, n1));
			PutKeyData(parentPos, parent);
			PutDbfNo(
			    parentPos, parent, GetDbfNo(n1->Leaf.NoOfKeysThisNode - 1, n1));
			n1->Leaf.NoOfKeysThisNode--;
		} else {
			...
			PutKeyData(n1->Leaf.NoOfKeysThisNode, n1);
			PutDbfNo(
			    n1->Leaf.NoOfKeysThisNode, n1, GetDbfNo(parentPos, parent));
			n1->Leaf.NoOfKeysThisNode++;

			// 8/20/03 gkunkel n1LastLeftNodeNo = GetLeftNodeNo(medianOffset,
			// n2);
			PutLeftNodeNo(
			    n1->Leaf.NoOfKeysThisNode, n1, GetLeftNodeNo(medianOffset, n2));
			...
			// Still investigating the -1 thing with clipper, If anyone has
			// clues, please let me know - bob@synxis.com
			...
			//            xbShort clipperMessedUpIndex =
			//            n1->Leaf.NoOfKeysThisNode;
			for (i = n1->Leaf.NoOfKeysThisNode, j = 0; j < medianOffset;
			     i++, j++) {
				...
				//        // Clipper, don't know why
				//        PutLeftNodeNo(0, n2 , -1 );
				//        std::cout << "Clipper hack in loop i = " <<  i <<
				//        std::endl;
				//      }
...
xbShort xbNtx::CheckIndexIntegrity(const xbShort option) {
	...
	if (option)
		std::cout << "Checking NTX " << GetFileName() << std::endl;
	rc = dbf->GetRecord(ctr);
	while (ctr < dbf->NoOfRecords()) {
		ctr++;
		if (option)
			std::cout << "Checking Record " << ctr << std::endl;
