...
xbShort xbExpn::BuildExpressionTree(const char * Expression,
	...
	while (rc == 0) {
		...
		if (TokenType != 'O') {
			...
			if (TokenType == 'E') {
				if ((rc = ReduceComplexExpression(p, TokenLen, CurNode, d)) !=
				    0)
					return rc;
				if (PreviousNode)
		...
		//      p += CurNode->Len;   // 2/20/04 - not sure when this was updated
		//      - gk
		p += LocTokenLen;
		...
		//    TokenLenCtr += CurNode->Len;  // 2/20/04 - not sure when this was
		//    updated - gk
...
char xbExpn::GetExpressionResultType(xbExpNode * e) {
	...
	if (Temp->Type == 'O' && *Temp->NodeText == '-' && Temp->Sibling1 &&
	    Temp->Sibling2 && Temp->Sibling1->ExpressionType == 'D' &&
...
xbShort xbExpn::OperatorWeight(const char * Oper, xbShort len) {
	...
	*/
	...
	if (strncmp(Oper, ".AND.", 5) == 0 || strncmp(Oper, ".OR.", 4) == 0 ||
	    strncmp(Oper, ".NOT.", 5))
		return 10;
...
void xbExpn::DumpExpressionTree(xbExpNode * E, xbShort printOption) {
	...
	if (E->Sibling1)
		DumpExpressionTree(E->Sibling1, printOption);
	if (E->Sibling2)
		DumpExpressionTree(E->Sibling2, printOption);
	if (E->Sibling3)
		DumpExpressionTree(E->Sibling3, printOption);
	return;
}
/*************************************************************************/
//! DumpExpNode
