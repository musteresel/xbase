...
/*!
  ...
  name (TableName) and schema (xbSchema s).  The OverLay switch is used to
  determine if an existing file should be overwritten or an error flagged if the
  ...
  \param Overlay One of the following:
    \htmlonly
      ...
      <table border=2><tr><th>OverLay</th><th>Description</th></tr>
        <tr><td>XB_OVERLAY</td><td>Overwrite existing file if it
  exists</td></tr> <tr><td>XB_DONTOVERLAY</td><td>Report an error if file
  exists</td></tr>
      </table>
    \endhtmlonly
    \latexonly
  ...
  \returns One of the following return codes:
    \htmlonly
      ...
      <table border=2><tr><th>Return Code</th><th>Description</th></tr>
        ...
        <tr><td>XB_FILE_EXISTS</td><td>If the file exists and OverLay is
  XB_DONTOVERLAY</td></tr> <tr><td>XB_OPEN_ERROR</td><td>Couldn't open the
  file</td></tr>            <tr><td>XB_NO_MEMORY</td><td>Memory allocation
  error</td></tr> <tr><td>XB_WRITE_ERROR</td><td>Couldn't write to disk</td><tr>
      </table>
    \endhtmlonly
    \latexonly
      \\
      ...
      \begin{tabular}{|l|l|} \hline
        ...
        XB\_FILE\_EXISTS & If the file exists and OverLay is XB\_DONTOVERAY \\
  \hline XB\_OPEN\_ERROR & Couldn't open the file \\ \hline XB\_WRITE\_ERROR &
  Couldn't write to disk \\ \hline \end{tabular} \endlatexonly
*/
...
//      return rc;
	...
	while (s[i].Type != 0) {
		...
		if (s[i].Type != 'C' && s[i].Type != 'N' && s[i].Type != 'F' &&
		    s[i].Type != 'D' &&
#ifdef XB_MEMO_FIELDS
		    s[i].Type != 'M' &&
#endif /* XB_MEMO_FIELDS */
		...
		//      if( !MemoSw && ( s[i].Type=='M' || s[i].Type=='B' ||
		//      s[i].Type=='O'))
	...
	if ((SchemaPtr = (xbSchemaRec *)malloc(NoOfFields * sizeof(xbSchemaRec))) ==
	    NULL) {
		free(RecBuf);
		free(RecBuf2);
		fclose(fp);
...
xbShort xbDbf::DumpHeader(xbShort Option) {
	...
	if (Option != 2) {
		...
		if (Version == 3)
			...
			std::cout << "Dbase III file with memo fields" << std::endl
			          << std::endl;

		std::cout << "Last update date = " << (int)UpdateMM << "/"
		          << (int)UpdateDD << "/" << (int)UpdateYY % 100 << std::endl;

		std::cout << "Header length    = " << HeaderLen << std::endl;
		std::cout << "Record length    = " << RecordLen << std::endl;
		std::cout << "Records in file  = " << NoOfRecs << std::endl
		          << std::endl;
#ifdef XB_REAL_DELETE
		std::cout << "First Free Rec   = " << FirstFreeRec << std::endl
		          << std::endl;
#endif
	}
	if (Option != 1) {
...
#ifdef XB_MEMO_FIELDS
		if (!MemoSw && (SchemaPtr[i].Type == 'M' || SchemaPtr[i].Type == 'B' ||
		                SchemaPtr[i].Type == 'O'))
			MemoSw++;
#endif
	}
...
/* check for any duplicate keys */
	...
	while (i) {
		if (!i->index->UniqueIndex()) /* if we didn't prepare the key */
			if ((rc = i->index->CreateKey(0, 0)) !=
			    XB_NO_ERROR) /* then do it before the add    */
				return rc;
	...
	if (RealDelete && FirstFreeRec) {
		...
		*/
		if (_fseek(fp,
		           (HeaderLen + (((xbOffT)FirstFreeRec - 1) * RecordLen) + 1),
		           0) != 0)
...
xbShort xbDbf::GetRecord(xbULong RecNo) {
	...
	if (_fseek(
	        fp, (HeaderLen + (((xbOffT)RecNo - 1L) * RecordLen)), SEEK_SET)) {
#ifdef XB_LOCKING_ON
//    LockDatabase( XB_UNLOCK, RecNo );
#endif
		return XB_SEEK_ERROR;
...
/*!
 */
xbShort xbDbf::PackDatafiles(void (*statusFunc)(xbLong itemNum,
                                                xbLong numItems)) {
	xbShort rc, i;
	FILE * t;
	xbLong l;
	...
	if ((MemoFields = MemoFieldsPresent()) > 0) {
		...
		if (MemoHeader.Version == 0x03) {
			...
			for (i = 22; i < MemoHeader.BlockSize; i++)
				fputc(0x00, t);
		}
...
#ifdef XB_MEMO_FIELDS
			BufSize = 0L;
			//     Buf = NULL;   Already set to 0, this statement flags as
			//     memory leak
...
/*!
  ...
  \param LockWaitOption One of the following:
    \htmlonly
      ...
      <table border=2><tr><th>LockWaitOption</th><th>Description</th></tr>
        <tr><td>F_SETLK</td><td>Return immediately if the DBF file cannot be
  locked</td></tr> <tr><td>XB_LOCK</td><td>Wait for lock on DBF file to
  succeed</td></tr>
      </table>
    \endhtmlonly
    \latexonly
  ...
  \returns One of the following return codes:
    \htmlonly
      ...
      <table border=2><tr><th>Return Code</th><th>Description</th></tr>
        <tr><td>XB_NO_ERROR</td><td>No error</td></tr>
   <tr><td>XB_CLOSE_ERROR</td><td>Unable to close intermediate work
  file</td></tr> <tr><td>XB_OPEN_ERROR</td><td>Could not open file</td></tr>
   <tr><td>XB_NO_MEMORY</td><td>Memory allocation error</td></tr>
...
/*!
 */
xbShort xbDbf::RebuildAllIndices(void (*statusFunc)(xbLong itemNum,
                                                    xbLong numItems)) {
#if defined(XB_INDEX_ANY)
	xbShort rc;
	xbIxList * n;
