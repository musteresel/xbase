...
void xbNdx::DumpHdrNode(xbShort opt) {
	if (opt) {
		FILE * log;
		if ((log = fopen("xbase64.log", "a+t")) == NULL)
			return;
...
xbShort xbNdx::GetHeadNode(void) {
	...
	p = Node;
	HeadNode.StartNode = dbf->xbase->GetLong(p);
	p += 4;
	HeadNode.TotalNodes = dbf->xbase->GetLong(p);
	p += 4;
	HeadNode.NoOfKeys = dbf->xbase->GetLong(p);
	p += 4;
	HeadNode.KeyLen = dbf->xbase->GetShort(p);
	p += 2;
	HeadNode.KeysPerNode = dbf->xbase->GetShort(p);
	p += 2;
	HeadNode.KeyType = dbf->xbase->GetShort(p);
	p += 2;
	HeadNode.KeySize = dbf->xbase->GetLong(p);
	p += 4;
	HeadNode.Unknown2 = *p++;
	HeadNode.Unique = *p++;
...
xbLong xbNdx::GetDbfNo(xbShort RecNo, xbNdxNodeLink * n) {
	...
	if (!n)
		return 0L;
	temp = &n->Leaf;
	if (RecNo < 0 || RecNo > (temp->NoOfKeysThisNode - 1))
		return 0L;
	p = temp->KeyRecs + 4;
...
xbShort xbNdx::BSearchNode(const char * key,
	...
	if (c == 1)
		while (p < node->Leaf.NoOfKeysThisNode &&
		       (c = CompareKey(
		            key, GetKeyData(p, (xbNdxNodeLink *)node), klen)) == 1)
			p++;
	...
	*comp = c;
	...
	if (!c)
		while (p > 0 &&
		       !CompareKey(key, GetKeyData(p - 1, (xbNdxNodeLink *)node), klen))
			p--;
...
xbShort xbNdx::CreateIndex(const char * IxName,
	...
	*/
	...
	//   HeadNode.KeysPerNode = (xbUShort) ( XB_NDX_NODE_SIZE - (2*sizeof(
	//   xbLong ))) /
	//      (HeadNode.KeyLen + 8 );
	 ...
	 */
	//   while(( HeadNode.KeyLen % 4 ) != 0 ) HeadNode.KeyLen++;  /* multiple of
	//   4*/ HeadNode.KeySize = HeadNode.KeyLen + 8;
	...
	*/
	...
	HeadNode.KeySize = HeadNode.KeyLen + 8;
	while ((HeadNode.KeySize % 4) != 0)
		HeadNode.KeySize++; /* multiple of 4*/

	HeadNode.KeysPerNode =
	    (xbUShort)(XB_NDX_NODE_SIZE - (2 * sizeof(xbLong))) / HeadNode.KeySize;
...
xbShort xbNdx::PutKeyInNode(xbNdxNodeLink * n,
	...
	if (GetLeftNodeNo(0, n))
		PutLeftNodeNo(n->Leaf.NoOfKeysThisNode + 1,
		              n,
...
xbShort xbNdx::SplitLeafNode(xbNdxNodeLink * curNode,
	...
	if (pos < curNodeNewSize) { /* new key goes in current node */
		...
		for (i = startPos, j = 0; i < CurNode->Leaf.NoOfKeysThisNode;
		     i++, j++) {
		...
		if (pos) {
			...
			*/
			...
			for (i = startPos, j = 0;
			     j < pos && i < curNode->Leaf.NoOfKeysThisNode;
			     i++, j++) {
				memcpy(KeyBuf, GetKeyData(i, curNode), HeadNode.KeyLen);
				PutKeyData(j, newNode);
				PutDbfNo(j, newNode, GetDbfNo(i, curNode));
...
xbShort xbNdx::SplitINode(xbNdxNodeLink * nodeToSplit,
	...
	if (pos < (newNodeToSplitSize - 1)) {
		...
		for (i = startPos, j = 0; i <= nodeToSplit->Leaf.NoOfKeysThisNode;
		     i++, j++) {
	...
	/*************/
		...
		/* do part one of the key migration */
		if (pos) {
			//      startPos = nodeToSplit->Leaf.NoOfKeysThisNode -
			//      newNodeToSplitSize + 2;

			// 5/29/04 gak changed the following line for index probs
			//      startPos = nodeToSplit->Leaf.NoOfKeysThisNode -
			//      newNodeToSplitSize + 1;
			...
			startPos = nodeToSplit->Leaf.NoOfKeysThisNode - newNodeToSplitSize +
			           offset;
	...
	if ((rc = PutLeafNode(nodeToSplit->NodeNo, nodeToSplit)) != 0)
		return rc;
	if ((rc = PutLeafNode(newNode->NodeNo, newNode)) != 0)
		return rc;
	return 0;
...
xbShort xbNdx::CheckIndexIntegrity(const xbShort option) {
	...
	if (option)
		std::cout << std::endl
		          << "Total records checked = " << ctr - 1 << std::endl;
	return XB_NO_ERROR;
}
#endif
/***********************************************************************/
...
xbShort xbNdx::ReIndex(void (*statusFunc)(xbLong itemNum, xbLong numItems)) {
	...
	for (l = 1; l <= dbf->PhysicalNoOfRecords(); l++) {
		if (statusFunc &&
		    (l == 1 || !(l % 100) || l == dbf->PhysicalNoOfRecords()))
