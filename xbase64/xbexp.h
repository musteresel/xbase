...
public:
	...
	xbExpNode * GetFirstTreeNode() { return GetFirstTreeNode(Tree); }
	...
	char GetExpressionResultType() { return GetExpressionResultType(Tree); }
	char * GetCharResult();
	...
	xbShort BuildExpressionTree(const char * Expression,
	                            xbShort MaxTokenLen,
	                            xbDbf * d);
...
protected:
	...
	xbExpNode * LoadExpNode(const char * ENodeText,
	                        const char EType,
	                        const xbShort ELen,
	                        const xbShort BufLen);
	xbShort OperatorWeight(const char * Oper, xbShort len);
	xbShort ReduceComplexExpression(const char * NextToken,
	                                xbShort Len,
	                                xbExpNode * cn,
	                                xbDbf * d);
...
private:
	...
	//   static xbString DefaultDateFormat;  /*default date format for DTOC
	//   func*/
