...
public:
	...
	/* don't uncomment next line - it causes seg faults for some undiagnosed
	 * reason*/
	//   ~NDX() { if( NdxStatus ) CloseIndex(); }

	xbShort CreateIndex(const char * IxName,
	                    const char * Exp,
	                    xbShort Unique,
	                    xbShort OverLay);
...
protected:
	...
	/*!
	 */
	inline xbShort CompareKey(const char * Key1,
	                          const char * Key2,
	                          xbShort Klen) {
		xbDouble d1, d2;
		int c;
	...
	xbShort BSearchNode(const char * key,
	                    xbShort klen,
	...
	xbShort FindKey(const char * Tkey,
	                xbLong DbfRec); /* for a specific dbf no */
};
#endif /* __XB_NDX_H__ */
